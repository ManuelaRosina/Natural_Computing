...
def reduceDimensions(coords):
    ...
    for coord in coords:
        if (
                coord['x'] > minimum + 7*virtual_view_unit and 
                coord['x'] < maximum - 7*virtual_view_unit and 
                coord['y'] > minimum + 7*virtual_view_unit and 
                coord['y'] < maximum - 7*virtual_view_unit
            ):
...
if __name__ == '__main__':
    coords = readCoords()
    coords, summary = reduceDimensions(coords)
    outputTxt(str(summary), 'summary')
    ant_count = 80
    generations = 150
    alpha_vals = [0.4, 0.6, 0.8]
    beta = 2
    pheromone_residual_coefficients = [0.4, 0.6]
    pheromone_residual_coefficients_local = [0.4, 0.6]
    q_0 = 0.2
